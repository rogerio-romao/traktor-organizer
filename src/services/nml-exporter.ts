...
export function buildPlaylistNml(playlistName: string, tracks: TrackRow[]): string {
    ...
    const collectionEntries = tracks
        .map((t) => {
            ...
            const musicalKey =
                t.musicalKeyValue === null
                    ? ''
                    : `\n    <MUSICAL_KEY${attr('VALUE', t.musicalKeyValue)}></MUSICAL_KEY>`;

            const loudness =
                t.loudnessPeak !== null ||
                t.loudnessPerceived !== null ||
                t.loudnessAnalyzed !== null
    ...
    <INFO${attr('BITRATE', t.bitrate)}${attr('GENRE', t.genre)}${attr('COMMENT', t.tags.length > 0 ? t.tags.join(' ') : t.commentRaw)}${attr('COVERARTID', t.coverArtId)}${attr('KEY', t.musicalKey)}${attr('KEY_LYRICS', t.keyLyrics)}${attr('PLAYTIME', t.duration)}${attr('PLAYTIME_FLOAT', t.durationFloat)}${attr('RANKING', ranking)}${attr('IMPORT_DATE', t.importDate)}${attr('RELEASE_DATE', t.releaseDate)}${attr('FILESIZE', t.filesize)}${attr('LABEL', t.label)}${attr('REMIXER', t.remixer)}${attr('PRODUCER', t.producer)}${attr('MIX', t.mix)}${attr('CATALOG_NO', t.catalogNo)}${attr('PLAYCOUNT', playcount)}${attr('LAST_PLAYED', t.lastPlayed)}${attr('FLAGS', t.flags)}${attr('COLOR', t.color)}></INFO>${tempo}${musicalKey}${loudness}
</ENTRY>`;
        })
        .join('\n');
