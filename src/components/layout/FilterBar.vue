<script setup lang="ts">
    ...
    const sourceTracks = computed(() =>
        activePlaylist.value ? playlistTracks.value : tracksStore.allTracks,
    );
    ...
    const isModified = computed(
        () =>
            Boolean(activePlaylist.value) &&
            (hasRemovals.value ||
                hasPendingUpdate.value ||
                hasTrackEdits.value ||
                hasAnyFilter.value ||
                Boolean(tracksStore.globalSearch.trim())),
    );

    // oxlint-disable-next-line max-statements
    function getDefaultPlaylistName(): string {
        const hasSearch = Boolean(tracksStore.globalSearch.trim());
        ...
        const nonTagCount = [hasSearch, hasGenre, hasKey, hasRating].filter(
            Boolean,
        ).length;

        // Any number of tags with no other filters → join with '-'
        if (nonTagCount === 0 && tagCount > 0)
            return tracksStore.activeTagFilters.join('-');
        ...
        if (nonTagCount === 1 && tagCount === 0) {
            ...
            const { genreFilter, keyFilter } = tracksStore;
            if (hasGenre) return genreFilter ?? '';
            if (hasKey)
                return `${formatKey(keyFilter ?? '', 'standard')} Scale`;
    ...
    function saveAsPlaylist(): void {
        ...
        const hasFilters =
            Boolean(filters.globalSearch.trim()) ||
            filters.activeTagFilters.length > 0 ||
            filters.genreFilter !== null ||
            filters.keyFilter !== null ||
    ...
    async function handleUpdatePlaylist(): Promise<void> {
    ...
    const genres = computed(() => {
        const unique = new Set(
            sourceTracks.value.map((t) => t.genre).filter(Boolean),
        );
    ...
    const keys = computed(() => {
        const unique = new Set(
            sourceTracks.value.map((t) => t.musicalKey).filter(Boolean),
        );
        return [...unique].toSorted((a, b) =>
            (formatKey(a, 'standard') || a).localeCompare(
                formatKey(b, 'standard') || b,
            ),
        );
    });
</script>
    ...
    <div class="filter-bar">
        <div class="filter-controls">
            ...
            <select
                ...
                @change="
                    tracksStore.keyFilter =
                        ($event.target as HTMLSelectElement).value || null
                "
            >
                <option value="">Key</option>
                <option v-for="k in keys" :key="k" :value="k">{{
                    formatKey(k, 'standard')
                }}</option>
            </select>
        ...
        <div v-if="tracksStore.activeTagFilters.length > 0" class="tag-pills">
            ...
            >
                ...
                <button
                    class="filter-remove"
                    @click="
                        tracksStore.activeTagFilters.splice(
                            tracksStore.activeTagFilters.indexOf(tag),
                            1,
                        )
                    "
                    >✕</button
