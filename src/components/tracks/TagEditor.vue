<script setup lang="ts">
    ...
    const track = computed(() =>
        tracksStore.allTracks.find((t) => t.id === trackId.value),
    );

    const normalizedInput = computed(() => normalizeTag(inputValue.value));
    ...
    const suggestions = computed(() => {
        ...
        return tagsStore.allTags
            .filter(
                (t) =>
                    !currentTags.has(t.name) &&
                    (!normalizedInput.value ||
                        t.name.includes(normalizedInput.value)),
            )
            .slice(0, 8);
    });
    ...
    function onBackdropClick(e: MouseEvent): void {
        if (
            (e.target as HTMLElement).classList.contains('tag-editor-backdrop')
        ) {
            close();
        }
    }
        ...
        >
            <div class="tag-editor-dialog">
                ...
                <div class="tag-editor-body">
                    <div
                        class="current-tags"
                        v-if="track && track.tags.length > 0"
                    >
                        <span
                        ...
                        <ul v-if="showSuggestions" class="suggestions">
                            ...
                            >
                                <span
                                    ...
                                    :style="{
                                        background: getTagColor(s.name).bg,
                                        borderColor: getTagColor(s.name).border,
                                    }"
                                    >{{ s.name }}</span
                                >
